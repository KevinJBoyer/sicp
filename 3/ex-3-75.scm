...
(define (make-zero-crossings-average input-stream last-value last-average)
  (let ((avpt (/ (+ (stream-car input-stream)
                    last-value)
                 2)))
    (cons-stream
     (sign-change-detector avpt last-average)
     (make-zero-crossings-average
      (stream-cdr input-stream) (stream-car input-stream) avpt))))

(stream-print-first (make-zero-crossings-average sensor-data 0 (stream-car sensor-data)) 11)