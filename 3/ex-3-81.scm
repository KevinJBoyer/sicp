...
(define (rand-stream rand-init requests)
  (cons-stream
   rand-init
   (rand-stream
    (if (eq? (car (stream-car requests)) 'generate)
        (stream-car (rand-stream (rand-update rand-init) the-empty-stream))
        (stream-car (rand-stream (cdr (stream-car requests)) the-empty-stream)))
    (stream-cdr requests))))

(stream-print-first (rand-stream 5
                                 (stream-from-list (list
                                                    (cons 'generate 0) ; 5
                                                    (cons 'reset 7) ; 3 - junk
                                                    (cons 'generate 0) ; 7
                                                    (cons 'generate 0) ; 71
                                                    (cons 'reset 7) ; 47 - junk
                                                    (cons 'generate 0) ; 7
                                                    (cons 'generate 0) ; 71?
                                                    ))
                                 )
