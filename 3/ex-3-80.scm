...
(define (weighted-pairs-i-less-j s t weight)
  (cons-stream
   ...
   (merge-weighted
    (stream-map (lambda (x) (list (stream-car s) x))
                (stream-cdr t))
    (weighted-pairs-i-less-j (stream-cdr s) (stream-cdr t) weight)
    weight
    )))
...
;;;
(define (cube x) (* x x x))
(define (sum-cubes x y) (+ (cube x) (cube y)))

(define (weight-by-sum-of-cubes s1-pair s2-pair)
  (let ((s1 (sum-cubes (car s1-pair) (cadr s1-pair)))
        (s2 (sum-cubes (car s2-pair) (cadr s2-pair))))
    (cond ((< s1 s2) -1)
          ((> s1 s2) 1)
          (else 0))))

; WIP....
(define (stream-duplicate-pairs s weight)
  (define (stream-duplicates-helper s weight prev)
    (cond
      ((stream-null? s) the-empty-stream)
      ((= 0 (weight prev (stream-car s)))
       (cons-stream
        prev
        (stream-duplicates-helper (stream-cdr s) weight (stream-car s))))
       (else (stream-duplicates-helper (stream-cdr s) weight (stream-car s)))))
    (stream-duplicates-helper (stream-cdr s) weight (stream-car s)))
...
(define sensor-data (stream-from-list (list 1 2 1.5 1 .5 -.1 -2 -3 -2 -.5 .2 3 4)))
;(stream-print-first (zero-crossings sensor-data) 11)

;(display-line "")

(define (zero-crossings-new sense-data)
  (stream-map sign-change-detector
              sense-data
              (cons-stream 0 sense-data)
              ))

;(stream-print-first (zero-crossings-new sensor-data) 11)
...
;(stream-print-first (zero-crossings-new (smooth sensor-data)) 10)
...
; I can't check this, because the above code gives the error
; "y: undefined; cannot use before initialization" in Racket.

; SICP itself warns:
; "This procedure is not guaranteed to work in all Scheme implementations, although
...
; Not satisfying, but I think this is correct:
...
; Perhaps I'll come back after getting through Section 4.1.6 and see if I can identify
; the "simple variation" needed :-)

;;;

; ex 3.78
; I similarly can't execute this. My answer at least matches those online, but I'm
; skeptical -- I got here by pattern matching with the graphic of the diagram.
...
; Test case provided online. Can't execute unfortuntaely :-(
...
; ex 3.79
; Again, I can't execute, so comparing my answers against those online
; I had `(define ddy (stream-map f dy))` originally, but it makes sense
; that f can depend on both dy and y
...
; Oh my god! Karthikk provides a solution to executing these at:
; http://community.schemewiki.org/?sicp-ex-3.80

; That said I'm somewhat let down by this set of exercises. I'm not
; sure if I'm missing something here, but usually each successive exercise
; is carefully crafted to push you just a little bit further in your understanding.
; Here I felt like 3.78, 3.79, and 3.80 were all accomplished (on my end)
; by pattern-matching the diagrams. There was a bit of a "click"
; on second-order differential equations when I saw the form of `solve-2nd`
; but I'm not sure what I was supposed to be taking away from 3.80.