#lang sicp

(#%require "type-table.scm")
(#%require "ex-2-85-apply-generic.scm")
(define (square n) (* n n))

(define (add x y) (apply-generic 'add x y))
(define (sub x y) (apply-generic 'sub x y))
(define (mul x y) (apply-generic 'mul x y))
(define (div x y) (apply-generic 'div x y))

; todo: for scheme-numbers and for rational numbers
(define (sqrt-gen x) (apply-generic 'sqrt-gen x))
(define (square-gen x) (apply-generic 'square-gen x))
(define (sin-gen x) (apply-generic 'sin-gen x))
(define (cos-gen x) (apply-generic 'cos-gen x))
(define (atan-gen x) (apply-generic 'atan-gen x))

(put 'sqrt-gen '(scheme-number) (lambda (x) (make-scheme-number (sqrt x))))
(put 'sqrt-gen '(rational)
     (lambda (x)
       (make-scheme-number (/ (sqrt (car x))
                              (sqrt (cdr x))))))
(put 'square-gen '(scheme-number) (lambda (x) (make-scheme-number (* x x))))
(put 'square-gen '(rational)
     (lambda (x)
       (make-rational (* (car x) (car x))
                      (* (cdr x) (cdr x)))))
(put 'sin-gen '(scheme-number) (lambda (x) (make-scheme-number (sin x))))
(put 'sin-gen '(rational) (lambda (x) (make-scheme-number (sin (/ (car x) (cdr x))))))
(put 'cos-gen '(scheme-number) (lambda (x) (make-scheme-number (cos x))))
(put 'cos-gen '(rational) (lambda (x) (make-scheme-number (cos (/ (car x) (cdr x))))))
(put 'atan-gen '(scheme-number) (lambda (x) (make-scheme-number (atan x))))
(put 'atan-gen '(rational) (lambda (x) (make-scheme-number (atan (/ (car x) (cdr x))))))


(define (install-rectangular-package)
  ;; internal procedures
  (define (real-part z) (car z))
  (define (imag-part z) (cdr z))
  (define (make-from-real-imag x y) (cons x y))
  (define (magnitude z)
    (sqrt-gen (add (square-gen (real-part z))
                   (square-gen (imag-part z)))))
  (define (angle z)
    (atan (imag-part z) (real-part z)))
  (define (make-from-mag-ang r a) 
    (cons (mul r (cos-gen a)) (mul r (sin-gen a))))

  ;; interface to the rest of the system
  (define (tag x) (attach-tag 'rectangular x))
  (put 'real-part '(rectangular) real-part)
  (put 'imag-part '(rectangular) imag-part)
  (put 'magnitude '(rectangular) magnitude)
  (put 'angle '(rectangular) angle)
  (put 'make-from-real-imag 'rectangular
       (lambda (x y) (tag (make-from-real-imag x y))))
  (put 'make-from-mag-ang 'rectangular
       (lambda (r a) (tag (make-from-mag-ang r a))))
  'done)

(define (install-polar-package)
  ;; internal procedures
  (define (magnitude z) (car z))
  (define (angle z) (cdr z))
  (define (make-from-mag-ang r a) (cons r a))
  (define (real-part z)
    (mul (magnitude z) (cos-gen (angle z))))
  (define (imag-part z)
    (mul (magnitude z) (sin-gen (angle z))))
  (define (make-from-real-imag x y) 
    (cons (sqrt-gen (add (square-gen x) (square-gen y)))
          (atan-gen y x)))

  ;; interface to the rest of the system
  (define (tag x) (attach-tag 'polar x))
  (put 'real-part '(polar) real-part)
  (put 'imag-part '(polar) imag-part)
  (put 'magnitude '(polar) magnitude)
  (put 'angle '(polar) angle)
  (put 'make-from-real-imag 'polar
       (lambda (x y) (tag (make-from-real-imag x y))))
  (put 'make-from-mag-ang 'polar
       (lambda (r a) (tag (make-from-mag-ang r a))))
  'done)

;(#%provide install-rectangular-package)
;(#%provide install-polar-package)


(install-rectangular-package)
(install-polar-package)
;;;;;;;;;;

;; Generic selectors

(define (real-part z) (apply-generic 'real-part z))
(define (imag-part z) (apply-generic 'imag-part z))
(define (magnitude z) (apply-generic 'magnitude z))
(define (angle z) (apply-generic 'angle z))


;; Constructors for complex numbers

(define (make-from-real-imag x y)
  ((get 'make-from-real-imag 'rectangular) x y))

(define (make-from-mag-ang r a)
  ((get 'make-from-mag-ang 'polar) r a))

;;;;;;;;;;;;;;;;;;;;;;;;;

(#%provide add)
(#%provide sub)
(#%provide mul)
(#%provide div)

(define (install-scheme-number-package)
  (define (tag x)
    (attach-tag 'scheme-number x))
  (put 'add '(scheme-number scheme-number)
       (lambda (x y) (tag (+ x y))))
  (put 'sub '(scheme-number scheme-number)
       (lambda (x y) (tag (- x y))))
  (put 'mul '(scheme-number scheme-number)
       (lambda (x y) (tag (* x y))))
  (put 'div '(scheme-number scheme-number)
       (lambda (x y) (tag (/ x y))))
  (put 'make 'scheme-number
       (lambda (x) (tag x)))
  'done)

(define (make-scheme-number n)
  ((get 'make 'scheme-number) n))

(#%provide make-scheme-number)

(define (install-rational-package)
  ;; internal procedures
  (define (numer x) (car x))
  (define (denom x) (cdr x))
  (define (make-rat n d)
    (let ((g (gcd n d)))
      (cons (/ n g) (/ d g))))
  (define (add-rat x y)
    (make-rat (+ (* (numer x) (denom y))
                 (* (numer y) (denom x)))
              (* (denom x) (denom y))))
  (define (sub-rat x y)
    (make-rat (- (* (numer x) (denom y))
                 (* (numer y) (denom x)))
              (* (denom x) (denom y))))
  (define (mul-rat x y)
    (make-rat (* (numer x) (numer y))
              (* (denom x) (denom y))))
  (define (div-rat x y)
    (make-rat (* (numer x) (denom y))
              (* (denom x) (numer y))))
  ;; interface to rest of the system
  (define (tag x) (attach-tag 'rational x))
  (put 'numer '(rational) numer)
  (put 'denom '(rational) denom)
  (put 'add '(rational rational)
       (lambda (x y) (tag (add-rat x y))))
  (put 'sub '(rational rational)
       (lambda (x y) (tag (sub-rat x y))))
  (put 'mul '(rational rational)
       (lambda (x y) (tag (mul-rat x y))))
  (put 'div '(rational rational)
       (lambda (x y) (tag (div-rat x y))))

  (put 'make 'rational
       (lambda (n d) (tag (make-rat n d))))
  'done)

(define (make-rational n d)
  ((get 'make 'rational) n d))

(#%provide make-rational)

(define (install-complex-package)
  ;; imported procedures from rectangular and polar packages
  (define (make-from-real-imag x y)
    ((get 'make-from-real-imag 'rectangular) x y))
  (define (make-from-mag-ang r a)
    ((get 'make-from-mag-ang 'polar) r a))
  ;; internal procedures
  (define (add-complex z1 z2) ;(display z1)(newline)(display z2)
    (make-from-real-imag (add (real-part z1) (real-part z2))
                         (add (imag-part z1) (imag-part z2))))
  (define (sub-complex z1 z2)
    (make-from-real-imag (sub (real-part z1) (real-part z2))
                         (sub (imag-part z1) (imag-part z2))))
  (define (mul-complex z1 z2)
    (make-from-mag-ang (mul (magnitude z1) (magnitude z2))
                       (add (angle z1) (angle z2))))
  (define (div-complex z1 z2)
    (make-from-mag-ang (div (magnitude z1) (magnitude z2))
                       (sub (angle z1) (angle z2))))

  ;; interface to rest of the system
  (define (tag z) (attach-tag 'complex z))
  (put 'add '(complex complex)
       (lambda (z1 z2) (tag (add-complex z1 z2))))
  (put 'sub '(complex complex)
       (lambda (z1 z2) (tag (sub-complex z1 z2))))
  (put 'mul '(complex complex)
       (lambda (z1 z2) (tag (mul-complex z1 z2))))
  (put 'div '(complex complex)
       (lambda (z1 z2) (tag (div-complex z1 z2))))
  (put 'make-from-real-imag 'complex
       (lambda (x y) (tag (make-from-real-imag x y))))
  (put 'make-from-mag-ang 'complex
       (lambda (r a) (tag (make-from-mag-ang r a))))
  'done)

(define (make-complex-from-real-imag x y)
  ((get 'make-from-real-imag 'complex) x y))

(define (make-complex-from-mag-ang r a)
  ((get 'make-from-mag-ang 'complex) r a))

(#%provide make-complex-from-real-imag)
(#%provide make-complex-from-mag-ang)

(#%provide install-scheme-number-package)
(#%provide install-complex-package)
(#%provide install-rational-package)

(define (numer x) (apply-generic 'numer x))
(define (denom x) (apply-generic 'denom x))

; add real numbers as a data type
(define (make-real n) (attach-tag 'real n))
(#%provide make-real)

; add coercions...
(define (scheme-number->rational n) (make-rational n 1))
(define (rational->real n) (make-real (/ (car n) (cdr n))))
(define (real->complex n) (make-complex-from-real-imag n 0))

(put 'raise '(scheme-number) scheme-number->rational)
(put 'raise '(rational) rational->real)
(put 'raise '(real) real->complex)

;;;;

(install-scheme-number-package)
(install-complex-package)
(install-rational-package)

;;;;

; some sample datums to test  with

(define n1 (make-scheme-number 11))
(define n2 (make-scheme-number 19))

(define rat1 (make-rational 3 5))
(define rat2 (make-rational 7 4))

(define real1 (make-real 1.1113))
(define real2 (make-real 3.4567))

(define c1 (make-complex-from-real-imag 13 17))
(define c2 (make-complex-from-mag-ang 5 3.14))
(define c3 (make-complex-from-real-imag -4 2))
(define c4 (make-complex-from-real-imag 51 52))

(#%provide n1)
(#%provide n2)
(#%provide rat1)
(#%provide rat2)
(#%provide real1)
(#%provide real2)
(#%provide c1)
(#%provide c2)
(#%provide c3)
(#%provide c4)

;;;;

(define (project-complex x) (make-real (real-part x)))
(define (project-real x) (make-rational (round x) 1))
(define (project-rational x) (make-scheme-number (quotient (numer (attach-tag 'rational x)) (denom (attach-tag 'rational x)))))

(put 'project '(complex) project-complex)
(put 'project '(real) project-real)
(put 'project '(rational) project-rational)

(define (project x) (apply-generic 'project x))
(#%provide project)

;;;;

(add (make-complex-from-real-imag (make-scheme-number 13) (make-rational 1 2))
     (make-complex-from-real-imag (make-scheme-number -5) (make-scheme-number 0)))